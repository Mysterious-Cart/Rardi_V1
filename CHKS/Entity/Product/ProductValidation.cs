...
public class ProductValidator : AbstractValidator<CreateProductRequest>
{
    public ProductValidator()
    {
        ...
        RuleFor(x => x.Name).Length(3, 50).WithMessage("Name must be between 3 and 50 characters");
        RuleFor(x => x.Name).Matches(@"^[a-zA-Z0-9\s]*$").WithMessage("Name must contain only letters and numbers");
        RuleFor(x => x.Name).Must(x => x != "Product").WithMessage("Name cannot be 'Product'");
        ...
        RuleFor(x => x.Export).GreaterThanOrEqualTo(0).WithMessage("Export cannot be negative");
        RuleFor(x => x.Export).LessThan(x => x.Import).WithMessage("Export cannot be greater than Import");

        RuleFor(x => x.Status).NotEmpty().WithMessage("Status is required");
        RuleFor(x => x.Status).IsInEnum().WithMessage("Status not recognized");
    }
}